...
import json
import pathlib
import M_04_functions as normalization_module

PATH = os.path.join(pathlib.Path.cwd(), 'newsfeed.txt')

class Publication:
    def __init__(self, data_type, text_str='', body='', path='', filename=''):
    ...
    def publishing(self, save_to_db = False):
        def p1():
            object = News(data_type=self.data_type, text_str=self.text_str, body=self.body, city=input('City: '))

            if save_to_db is True:
                Publication.write_to_db(self, [self.data_type, object.body])
                print('Data stored in db')

            return object.body

        def p2():
            object = PrivateAd(data_type=self.data_type, text_str=self.text_str, body=self.body)
        ...
        def p3():
            object = UsefulTips(data_type=self.data_type, text_str=self.text_str, body=self.body, author=input('Author: '))
...
class PrivateAd(Publication):
    def __init__(self, data_type, text_str, body, delta_time=''):
        super().__init__(data_type, text_str, body)
...
class PublFromXMLFile:
    def __init__(self, count_sentences=0, filename='', path=pathlib.Path.cwd()):
