...
for i in a.lower().split('\n'):  # Convert into lower case and split it by '\n'
    for y in i.split('.'):  # Split again by '.'
        ...
        # Create lists with every word of all sentences, including empty (when '/n')
        last_word = [i.replace(':', '') for i in y.split(' ') if i != '' and len(y) > 0]
...
last_words_sentance = ' '.join(last_words) + '.'  # Create a new sentence(string) from list 'last_w'
# The result
my_o = my_o[:my_o.find('paragraph.') + 10] + '\n' + last_words_sentance.capitalize() + my_o[
                                                                                       my_o.find('paragraph.') + 10:]
